  ...
  <div class="login-page">
    <q-form @submit="handleLogin" class="login-form">
      ...
      <div class="form-group">
        <q-input v-model="form.email" type="email" :label="$t('auth.email')" outlined dense :loading="authStore.loading"
          :error="!!errors.email" :error-message="errors.email" @update:model-value="clearError('email')"
          class="form-input">
      ...
      <div class="form-group">
        <q-input v-model="form.password" :type="showPassword ? 'text' : 'password'" :label="$t('auth.password')"
          outlined dense :loading="authStore.loading" :error="!!errors.password" :error-message="errors.password"
          @update:model-value="clearError('password')" class="form-input">
          <template v-slot:prepend>
            <q-icon name="lock" color="grey-6" />
          </template>
          <template v-slot:append>
            <q-icon :name="showPassword ? 'visibility_off' : 'visibility'" class="cursor-pointer" color="grey-6"
      ...
      <div class="demo-credentials">
        ...
        <div class="demo-accounts">
          <q-btn flat dense no-caps color="grey-7" @click="fillDemoCredentials('admin')" :disable="authStore.loading"
            class="demo-btn">
            {{ $t('auth.superAdmin') }}
          </q-btn>
          <q-btn flat dense no-caps color="grey-7" @click="fillDemoCredentials('owner')" :disable="authStore.loading"
            class="demo-btn">
            {{ $t('auth.owner') }}
          </q-btn>
          <q-btn flat dense no-caps color="grey-7" @click="fillDemoCredentials('user')" :disable="authStore.loading"
            class="demo-btn">
            {{ $t('auth.user') }}
          </q-btn>
        </div>
...
const validateForm = () => {
  ...
  if (!form.value.email) {
    newErrors.email = t('auth.emailIsRequired')
  ...
  if (!form.value.password) {
    ...
    newErrors.password = t('auth.passwordMustBeAtLeast')
  }
  ...
  try {
    ...
    if (result.success) {
      ...
      if (result.message?.includes('credentials')) {
        errors.value.password = t('auth.invalidCredentials')
      }
    }
  } catch (error) {
