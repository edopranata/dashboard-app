<template>
    <q-avatar :size="size" :class="avatarClass" :style="avatarStyle">
        <img v-if="avatarUrl" :src="avatarUrl" :alt="altText" @error="handleImageError" @load="handleImageLoad" />
        <q-icon v-else-if="!loading" name="person" :size="iconSize" class="text-grey-6" />
        <q-spinner v-if="loading" color="grey-5" :size="spinnerSize" />
    </q-avatar>
</template>
    ...
    setup(props, { emit }) {
        ...
        const loadAvatar = async () => {
            ...
            if (props.user?.avatar) {
                // Use same base URL as axios config
